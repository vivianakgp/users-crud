    ...
    <div className='userList container mt-5 pb-5'>
            ...
            users.map( user => (
                <div className='card col-8 mb-2  col-lg-3 p-3  m-2' key={user.id}>
                    ...
                    <p><i className='bi bi-envelope me-2'style={{ fontSize: 20 }}></i>{user.email}</p>
                    <p><i className='bi bi-calendar-date me-2' style={{ fontSize: 20 }}></i>{user.birthday}</p>
                    <div className='d-flex flex-row-reverse '>
                        <i className='bi bi-pencil  p-2 m-2' onClick={()=>selectedToEdite(user)} style={{ fontSize:23, color:'blue' }}></i>
                        <i className='bi bi-trash  p-2 m-2' onClick={()=>deleteUser(user.id)} style={{ fontSize:23, color:'red'}}></i>
                    </div>
